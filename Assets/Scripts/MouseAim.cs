...
public class MouseAim : MonoBehaviour
{
    ...
    private void UpdatePosition()
    {
        ...
        // 应用X轴翻转
        Vector2 processedMousePoint = mousePoint;
        if (flipXAxis)
        {
            // 以屏幕中心为对称轴进行翻转
            float flippedX = center.x * 2 - mousePoint.x;
            processedMousePoint = new Vector2(flippedX, mousePoint.y);
        }

        // 临时变量，如果没有超出distance距离，就用处理后的鼠标位置，
        // 否则用center到处理后的鼠标位置的单位向量×distance，再加上center坐标
        Vector2 temp = processedMousePoint;
        if (Vector2.Distance(processedMousePoint, center) > distance)
        {
            temp = (processedMousePoint - center).normalized * distance + center;
        }
